...
#
# TCG-emulated runs (PUNAR_RAM_ACCEL != kvm) NEVER fail the build: the
# numbers are labeled "(VM, emulated)" and indicative only
# (PERFORMANCE_BUDGETS.md §2.2/§5.2) — a ceiling breach is downgraded to a
# warning annotation.
#
...
if [ "${fail}" -eq 1 ]; then
    echo "==> FAIL: idle-RAM budget gate" >&2
    exit 1
fi
echo "==> PASS: idle-RAM budget gate"
