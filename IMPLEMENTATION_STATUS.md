...
| M12 — Network privacy prototype | Local network observability, project-route policy, relay abstraction, simulated or prototype private relay | Not specified in spec §76 | **Core implemented; milestone gate open** — `punar-netd`, typed policy compilation, nftables rendering/apply boundary, privacy-bounded connection view, CLI, event-driven reconciliation, image integration and simulated relay have host coverage, and the staged ARM64 daemon/socket run in the release VM. The planned `m12-check`/`PUNAR_M12_OK` enforcement exercise does not exist yet; real loopback allow/deny/control probes, the netd-to-AI-ledger join, real networking/VPN behavior and physical-device coverage are therefore not claimed. |
