...
jq_check "effective document: six named entries, full shape, override permitted everywhere" \
...
jq_check "status compliance: overall compliant, 6 capability rows, counter is a number" \
    "${RUN_DIR}/m4-status-a.json" \
    '.compliance.overall == "compliant"
     and (.compliance.capabilities | length) == 6
