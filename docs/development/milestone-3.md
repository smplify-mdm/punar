...
| 6 | Audit: `/var/log/punar/audit.jsonl`, dir 0750 root:punar, file 0640 root:punar (since F0, 2026-09-25: `root:punar-audit`, and `audit.tail` is scoped per caller — ipc.md §5.5, §6), writes only by punard; every mutation + every denial; schema-conformant with documented sentinels (`agt_none`, `project_id:"system"`); rotation OUT. |
