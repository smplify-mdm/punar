...
| 3 | remain within idle budget | **MET ON NATIVE ARM64 VM:** exact Apple-HVF candidate `cf522b…d19133` measured **1004 MB mean / 1005 MB max** against the unchanged 1024 MB target, down from the comparable 1210/1213 MB baseline. Four first-party services totaled 24 MB PSS; idle CPU, writes and zram also passed. The unaccelerated-VM renderer policy is runtime-proven and real-GPU paths explicitly clear it. This closes clean-VM DoD item 3, not Raspberry Pi or bare-metal performance acceptance. Re-measured 2026-09-06 on the freshly built desktop exercise image `12209b24…5b0444` (desktop + dev profiles, so a heavier image than the release candidate above): **938 MB mean / 939 MB max**, services 25 MB PSS, idle CPU 0.01% of one CPU against a 0.50% ceiling and 73728 bytes of first-party idle writes against a 98304-byte ceiling — `tests/performance/check-budgets.sh` passes outright on ARM64. |
...
62 reviewed app identities, including clearly labelled official web entries and
...
status. That is now closed on both architectures. The canonical x86 KVM
desktop gate passed the whole M2..M12 suite on run 33999044578, and a
local Apple-HVF ARM64 run of a freshly built `punar-desktop-arm64.qcow2`
(2026-09-06) passed every exercise: M2 41, M3 33, M4 29, M5 63, M6 56,
M7 89, M8 144, M9 138, M10 134, M11 74, M12 69, plus 131 desktop-surface,
5 wireless and 15 isolated surface-cost checks. Idle RAM measured
938 MB mean / 939 MB max against the 1024 MB target, four first-party
services summed 25 MB PSS, idle CPU peaked at 0.01% of one CPU against a
0.50% ceiling and first-party idle writes were 73728 bytes against a
98304-byte ceiling, so `tests/performance/check-budgets.sh` passes on
ARM64 as well. Hosted ARM CI still exposes no usable `/dev/kvm` and
honestly records that skip, so the ARM64 desktop and performance numbers
are Apple-HVF local evidence, not hosted-CI evidence. Because the
sandboxed mock can no longer reach punard, the M8
...
### 6.7 Desktop UX, from the first hands-on VM session (2026-09-06)

The user drove the ARM64 desktop image and filed seven items. Investigating
each against the tree mattered more than the list itself: three were built and
merely unreachable by pointer, one was a mistake in which image was booted, and
only three were genuinely missing.

**Item 1, login — NOT A GAP.** The dev/CI image was booted, and its profile
autologins. The release image runs the real greeter: first-run account creation
with a ten-character policy, then PAM login every boot, plus an
`ext-session-lock-v1` lock surface. `check-release-image.sh` A4 fails the build
if `[initial_session]` ever appears, with a mutation test proving it bites. The
investigation did surface a real neighbouring gap, now closed: nothing ever
locked an idle session.

**Done.** Idle auto-lock (hypridle, ext-idle-notify, ten minutes, routed to the
Punar lock surface; product bound asserted by new release-image assertion A13,
which the CI image deliberately overrides so a sixteen-minute measurement
window is not mistaken for a user who walked away). Restart / shut down / end
session in System Control's Power view, on both branches of `viewPower()`
because a VM reports no battery and takes the dashed one. Day and date in the
bar clock. Pointer create-and-reach for workspaces.

**Backed out.** Pointer window move/resize. `bindm` appears nowhere in this
tree, and `hl.dsp.window.drag()`/`resize()` with no arguments throws during
config evaluation — taking every bind defined after it, which silently removed
float, pin, centre and the resize submap. The shipped Hyprland 0.56.2 Lua stub
types every dispatcher as `fun(...)`, so it names them without giving argument
semantics, and there is no raw-dispatcher escape hatch. This needs a live
...
Hyprland desktop surface. The app catalog at 22 entries against
`docs/design/app-catalog.md`'s own ~40 target, with whole categories empty; the
mechanism is sound, the shortfall is sandbox-clean rows. Time-of-day wallpaper,
which needs the explicit no-timer performance contract at `Wallpaper.qml:8`
...
three theme substitutions, so one file serves paper and panel; 454 KB to 1.5 MB
against 306 KB to 4.9 MB for the four shipped JPEGs.

Measured, not assumed: boot-to-desktop is 16 s with a plate default, identical
to the JPEG it replaced, so a 1600x1000 SVG of ~460 contour paths costs nothing
at first paint.

Idle RAM moved with this work and is attributed by elimination, not by a
controlled A/B — single measurements on the same host: 938/939 MB before,
965/968 with hypridle added, 972-982 with the plate default. Roughly +27 MB for
the idle daemon and +11 MB for the plate, against the unchanged 1024 MB target.
Still passing, with headroom down from 86 MB to about 48 MB. Worth attributing
properly if it tightens further.

Known inconsistency: the greeter's first frame is a hardcoded `file://` path to
`stillpoint.jpg`, pinned absolutely because `Quickshell.shellDir` differs when
