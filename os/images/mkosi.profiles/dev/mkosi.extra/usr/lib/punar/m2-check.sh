...
hyprctl dispatch "hl.dsp.window.move({ out_of_group = true })" >/dev/null 2>&1
sleep 1
...
hyprctl dispatch "hl.dsp.window.center()" >/dev/null 2>&1
hyprctl dispatch "hl.dsp.window.pin()" >/dev/null 2>&1
sleep 1
check_eq "pinned after pin" true "$(active_field .pinned)"
hyprctl dispatch "hl.dsp.window.pin()" >/dev/null 2>&1
hyprctl dispatch "hl.dsp.window.float({ action = 'unset' })" >/dev/null 2>&1
sleep 1
...
# --- 5. named workspaces -----------------------------------------------------
hyprctl dispatch "hl.dsp.workspace.rename({ workspace = '1', name = 'Atlas' })" >/dev/null 2>&1
sleep 1
...
hyprctl dispatch "hl.dsp.focus({ workspace = 'name:Atlas' })" >/dev/null 2>&1
sleep 1
...
dispatch_special() {
    ...
    ds_i=0
    while [ "${ds_i}" -lt 10 ]; do
        if hyprctl dispatch "hl.dsp.workspace.toggle_special('${ds_pad}')" >/dev/null 2>&1; then
            return 0
        fi
        ds_i=$((ds_i + 1))
        sleep 0.2
...
if [ -n "${scratch_address}" ]; then
    hyprctl dispatch "hl.dsp.window.close({ window = 'address:${scratch_address}' })" >/dev/null 2>&1
fi
...
hyprctl dispatch "hl.dsp.workspace.rename({ workspace = '1' })" >/dev/null 2>&1
sleep 1
...
hyprctl dispatch "hl.dsp.exec_cmd('${SHELL_CMD}')" >/dev/null 2>&1
