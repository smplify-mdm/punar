...
qml_calls="$(rg -n '^[[:space:]]*Hyprland\.dispatch\(' shell/punar-shell -g '*.qml' || true)"
qml_bad="$(printf '%s\n' "${qml_calls}" \
    | rg -v '^shell/punar-shell/Services/HyprlandActions\.qml:.*Hyprland\.dispatch\(("hl\.dsp\.|expression \+)' \
    || true)"
if [ -n "${qml_bad}" ]; then
...
script_calls="$(rg -n \
    ...
    -g '*.sh' || true)"
script_bad="$(printf '%s\n' "${script_calls}" \
    | rg -v 'hyprctl[[:space:]]+dispatch[[:space:]]+"hl\.dsp\.' \
    || true)"
