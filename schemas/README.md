...
- Every schema declares an `$id` of the form:
  ...
  where `<domain>` is the directory under `schemas/` (`common`, `ai-agent`, `audit`, `capability`, `desired-state`, `install`, `network`, `pim`, `policy`, `project`, `update`) and `<name>` is the file basename. Example: `https://schemas.punar.dev/v1alpha1/audit/audit-event.json`.
