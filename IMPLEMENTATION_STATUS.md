...
| M12 — Network privacy prototype | Local network observability, project-route policy, relay abstraction, simulated or prototype private relay | Not specified in spec §76 | **Clean ARM64 runtime gate green locally; x86_64 + canonical CI pending** — the exact 2026-08-29 image (`sha256:62081fb3a5d7fbf58115c35d5b04a5eb6957caf5d101ebe0089eba85612c14c9`) emitted **`PUNAR_M12_OK` with 66 assertions**. The real same-user allow/deny/out-of-scope control probes, cgroup-v2 nft attachment, kernel socket-to-cgroup attribution across the user boundary, named counters, privacy-bounded connection view, netd-to-ledger/audit joins, malformed-data fail-safe, table self-heal, no-`CAP_SYS_PTRACE` confinement and `punar-m12.png` all passed. The whole desktop reached `PUNAR_DESKTOP_OK` in 16 s and measured 1210/1213 MB RAM plus 24 MB combined Punar PSS. Host format/clippy/test/schema/shell/QML gates are green. Verification remains open until a fresh x86_64 image and canonical dual-architecture CI emit the same verdict; real relay/VPN behavior, escape-proof network namespaces and physical-device coverage remain out. |
