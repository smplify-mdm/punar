...
# 3. Convert back to a compressed qcow2 for transfer: OUT_DIR/prepared.qcow2.
#
# It stops there on purpose: prepared.qcow2 is the release image plus the
# probe, with no account and no per-machine secret, so it can travel as a
# workflow artifact. The machine that measures then makes it an
# uncompressed raw disk (the same format as the other lane's), creates the
# first account on it (bench_run.py onboard), warms it with one signed-in
# session (bench_run.py warmup) and commits that into the raw disk; every
# run boots a fresh overlay of the result (tools/bench/README.md,
# "Reproducing"). The original release image is never modified.
set -euo pipefail
