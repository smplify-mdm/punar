...
back. *(Corrected 2026-08-26: an earlier draft of this section said "64 bits
of entropy", conflating the 64-character rendering with the key length and
understating it by a factor of four. **TO VERIFY** at the pin that
`systemd-cryptenroll --recovery-key` on the snapshot's systemd emits exactly
this shape; the confirmation grammar below depends on the group count.)*
...
| Escrow | **Not in MVP.** Organisation escrow of recovery keys is the obvious enterprise requirement, it needs the real control plane (user-blocked 4) and an identity provider (user-blocked 5), and it is a decision about who can decrypt an employee's laptop that should be made deliberately and not defaulted into now. Named in §13. |
...
{ "v": 1, "locale": "C.UTF-8", "keymap": "us",
  ...
  "diskEncrypted": true, "diskRecoveryKeyShown": true }
```
...
| I30 | `/var/lib/punar` exists, is `0700 root:root`, and is on the `/var` mount; `/var/lib/punar/install/seed.json` validates against `onboarding.md` §4.3's shape, and `diskEncrypted` is `true` for the encrypted lane and `false` for the opt-out lane (both are run). When an `oobe-answers.json` was supplied it is present and **byte-identical** to the file the answer disk carried. |
...
8. **No recovery-key escrow**, so an organisation cannot recover an
   employee's device. `onboarding.md` §1.8 assigns org-held escrow of the
   *disk* key to this document and the enrollment chain; this document
   declines to create one in the MVP. Blocked on user-blocked 4 and 5, and
   it is a decision about who may decrypt a person's laptop that should not
   be defaulted into.
...
| Recovery-key escrow to an organisation | Blocked on user-blocked 4 + 5. Ownership assigned here by `onboarding.md` §1.8; accepted, not exercised |
