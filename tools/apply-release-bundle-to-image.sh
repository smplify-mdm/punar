...
PAYLOAD_NAME="$(jq -er '.uefi_slots.b.payload.filename' "${RELEASE_DIR}/release.json")"
PAYLOAD_DIGEST="$(jq -er '.uefi_slots.b.payload.digest_sha256' "${RELEASE_DIR}/release.json")"
PAYLOAD_SIZE="$(jq -er '.uefi_slots.b.payload.size_bytes' "${RELEASE_DIR}/release.json")"
UNCOMPRESSED_DIGEST="$(jq -er '.uefi_slots.b.payload.uncompressed_digest_sha256' "${RELEASE_DIR}/release.json")"
UNCOMPRESSED_SIZE="$(jq -er '.uefi_slots.b.payload.uncompressed_size_bytes' "${RELEASE_DIR}/release.json")"
UKI_NAME="$(jq -er '.uefi_slots.b.boot_artifact.filename' "${RELEASE_DIR}/release.json")"
UKI_DIGEST="$(jq -er '.uefi_slots.b.boot_artifact.digest_sha256' "${RELEASE_DIR}/release.json")"
UKI_SIZE="$(jq -er '.uefi_slots.b.boot_artifact.size_bytes' "${RELEASE_DIR}/release.json")"
...
PY
        ...
        preferred_selector="punar_${PUNAR_VERSION}*.efi"
        ...
        # `preferred` is assessment-aware in the pinned systemd-boot: it
        # selects this version while its retry count is non-zero, but skips it
        # after +0-N so a good older UKI can win. `default` deliberately does
        # not apply that assessment and would keep selecting an exhausted
        # entry. The glob survives both counter renames and final blessing.
        printf "preferred %s\ntimeout 0\n" "${preferred_selector}" > "${work}/esp/loader/loader.conf.new"
        ...
        grep -Fxq "preferred ${preferred_selector}" "${work}/esp/loader/loader.conf"
