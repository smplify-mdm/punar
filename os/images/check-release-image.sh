...
# A22 (F0-S2): Yama lets a process ptrace-attach only to its own descendants.
...
if [ ! -f "${YAMA_CONF}" ]; then
    fail A22 'the Yama policy is missing: usr/lib/sysctl.d/50-punar-yama.conf'
else
    yama_value=$(sed -n \
        's/^[[:space:]]*kernel[./]yama[./]ptrace_scope[[:space:]]*=[[:space:]]*\([0-9][0-9]*\)[[:space:]]*$/\1/p' \
        "${YAMA_CONF}" | tail -n 1)
    if [ "${yama_value}" != 1 ]; then
        fail A22 "the Yama policy sets kernel.yama.ptrace_scope to '${yama_value}', not 1"
    fi
fi
...
if [ ! -f "${PUNARD_TMPFILES}" ]; then
    ...
    for audit_file in audit.jsonl audit.jsonl.1 audit.jsonl.lock; do
        if ! grep -Eq "^z[[:space:]]+/var/log/punar/${audit_file}[[:space:]]+0640[[:space:]]+root[[:space:]]+punar-audit([[:space:]]|\$)" \
                "${PUNARD_TMPFILES}"; then
            fail A23 "/var/log/punar/${audit_file} is not declared 0640 root:punar-audit"
        fi
    done
fi
for tmpfiles_dir in usr/lib/tmpfiles.d etc/tmpfiles.d; do
    ...
    if [ -n "${AUDIT_GRANTS}" ]; then
        fail A23 "a tmpfiles line gives the audit trail to another group: $(printf '%s' "${AUDIT_GRANTS}" | sed "s#${ROOT}/##g" | tr '\n' ' ')"
    fi
done
group_members() {
...
if [ ! -f "${ROOT}/etc/group" ]; then
    fail A23 '/etc/group is missing; the audit and administrator groups cannot be proven'
else
    case "$(group_members punar-audit)" in
        missing) fail A23 'the punar-audit group does not exist' ;;
        'members=') ;;
        *) fail A23 "the punar-audit group names members: $(group_members punar-audit)" ;;
    esac
fi

# A24 (F0-S1): the device-administrator group exists, so onboarding can put
...
if [ -f "${ROOT}/etc/group" ]; then
    case "$(group_members punar-admin)" in
        missing) fail A24 'the punar-admin group does not exist; onboarding could not make the first account an administrator' ;;
        'members=') ;;
        *) fail A24 "the punar-admin group names members in the image: $(group_members punar-admin)" ;;
    esac
fi
if [ -f "${ROOT}/etc/gshadow" ] \
    && awk -F: '($1 == "punar-admin" || $1 == "punar-audit") && $4 != "" { bad = 1 } END { exit bad ? 0 : 1 }' \
        "${ROOT}/etc/gshadow"; then
    fail A24 '/etc/gshadow names members of punar-admin or punar-audit'
fi
