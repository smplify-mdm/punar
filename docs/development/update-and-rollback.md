...
| HTTPS transport construction and fail-closed behavior | **CODE-PROVEN, not end-to-end network-proven** | Unit tests exercise fixed privacy-preserving paths, HTTPS-only/TLS floor, redirect refusal, response bounds, exact signature admission, and no local fallback, through the real `punar-fetch` helper protocol with a fixed fake downloader. The helper's sandbox (`punar-fetch@.service`) was run once under systemd 257 in a container: a real HTTPS transfer, DNS through the resolver stub, loopback and link-local refused, exposure 1.1. The canonical VM remains `-nic none`; production CDN/TLS and resumption still require §12.2 evidence. |
