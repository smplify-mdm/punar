...
| **Slack** | `slack` | `CURATED · HARDENED NATIVE`, `license: proprietary`, `publisher: upstream` — the x86_64 payload is Slack's official digest-pinned `.deb`; Punar never runs its maintainer scripts and launches it with an isolated app home. ARM64 uses an explicitly labelled web fallback because Slack publishes no ARM64 Linux package. `securitySensitive: true` — it renders untrusted remote content and may use camera and microphone for calls. Disclosure: *"Your workspace administrator can retain and export messages, including direct messages, under Slack's own policies. Punar has no visibility into and no control over that."* |
